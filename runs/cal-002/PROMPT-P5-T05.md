...
The working directory is the repository as of commit T04; the files your task lists as inputs are present on disk and you may read them. Test command: `python -m unittest tests.test_fees -v`
...
4. **One commit.** When the Done Check passes, commit everything with the message `T05: <one-line summary>`.
...
6. **Blocking.** On the 3rd failure, or if any rule above traps you, STOP and write `BLOCKED-T05.md` with exactly:
   - `## Task` — the task ID.
...
- OR: `BLOCKED-T05.md` exists in the required format and no partial changes are left uncommitted.
