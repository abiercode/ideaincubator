...
The working directory is the repository as of commit T02; the files your task lists as inputs are present on disk and you may read them. Test command: `python -m unittest tests.test_policy -v`
...
4. **One commit.** When the Done Check passes, commit everything with the message `T03: <one-line summary>`.
5. **Retry bound.** You get at most 3 attempts at a passing Done Check. Attempt = implement (or fix), run `python -m unittest tests.test_policy -v`, observe.
6. **Blocking.** On the 3rd failure, or if any rule above traps you, STOP and write `BLOCKED-T03.md` with exactly:
   - `## Task` — the task ID.
...
- OR: `BLOCKED-T03.md` exists in the required format and no partial changes are left uncommitted.
