...
The working directory is the repository as of commit T01; the files your task lists as inputs are present on disk and you may read them. Test command: `python -m unittest tests.test_records -v`
...
4. **One commit.** When the Done Check passes, commit everything with the message `T02: <one-line summary>`.
5. **Retry bound.** You get at most 3 attempts at a passing Done Check. Attempt = implement (or fix), run `python -m unittest tests.test_records -v`, observe.
6. **Blocking.** On the 3rd failure, or if any rule above traps you, STOP and write `BLOCKED-T02.md` with exactly:
   - `## Task` — the task ID.
...
- `python -m unittest tests.test_records -v` exits 0.
- `git diff` touches only permitted files.
- Exactly one new commit exists with the required message format.
- OR: `BLOCKED-T02.md` exists in the required format and no partial changes are left uncommitted.
