...
The working directory is an empty git repository. Test command: `python -m unittest tests.test_records -v`
...
4. **One commit.** When the Done Check passes, commit everything with the message `T01: <one-line summary>`.
...
6. **Blocking.** On the 3rd failure, or if any rule above traps you, STOP and write `BLOCKED-T01.md` with exactly:
   - `## Task` — the task ID.
...
- OR: `BLOCKED-T01.md` exists in the required format and no partial changes are left uncommitted.
