...
```
A small command-line tool for freelancers and small businesses that settles late-fee
arguments. You give it a CSV of invoices (amount, due date, payments received) and a
late-fee policy (grace period in days, monthly percentage, maximum cap, rounding rule),
and it tells you exactly what each client owes today, with the math shown line by line.
Late-fee math is genuinely confusing — people compute it differently and argue about it.
This tool applies one written policy consistently and shows its work.
```

Current incubator constraints:

```
- Must be buildable in at most 3 build tasks by one developer
- Must be a pure input -> output utility: no accounts, no auth, no encryption,
  no payments, no personal data storage
- Must produce deterministic output that automated tests can verify
- Must run entirely locally (command-line; no hosted services, no API keys)
- Must be releasable as open source
```
